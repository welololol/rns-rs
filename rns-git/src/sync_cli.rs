        ...
        while let Some(arg) = iter.next() {
            match arg.as_str() {
                ...
                other if other.starts_with('-') => {
                    return Err(Error::msg(format!("unknown sync option {other}")));
                }
                _ => {
                    if remote.replace(arg).is_some() {
                        return Err(Error::msg("sync takes exactly one repository URL"));
                    }
                }
            }
        }
...
fn usage() -> &'static str {
    "usage: rngit sync [--config DIR] [--rnsconfig DIR] [-i|--identity PATH] <rns://destination/group/repo>"
}
...
mod tests {
    ...
    #[test]
    fn rejects_missing_sync_remote() {
        assert!(SyncOptions::parse(Vec::<String>::new()).is_err());
    }
}
